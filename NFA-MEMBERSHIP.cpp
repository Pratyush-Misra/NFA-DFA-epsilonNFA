...
string INPUT;       // string whose membership needs to be checked in NFA
...
map<string, map<char, vt<string>>> NFA;  // storing the graph
...
void initialisation() {
    ...
    FOR(STATES) {
        string newState;
        int type, numOfEdges;   // number of edges for this state
        cin >> newState >> type >> numOfEdges;
        ...
        FOR(numOfEdges) {
            ...
            NFA[newState][INPUT_SYMBOL].pb(nbrState);
        }
    }

    /*
        Sample input for NFA substring of "101"
        2 4
        q0 0 3
        0 q0
        1 q0
...
// function to display the NFA
void display() {
    cout << "INITIAL STATE: " << INITIAL_STATE << endl;
    cout << "FINAL STATES: ";
    go(fs, FINAL_STATES)    cout << fs << " ";
    cout << endl;
    for(auto& [curr, nbrs] : NFA) {
        cout << curr << "-> ";
        for(auto& [ipt, nbr_vector] : nbrs) {
            cout << '(' << ipt << ',' << '[';
            go(nbr_string, nbr_vector)  cout << nbr_string << ',';
            cout << "])";
        }
        cout << endl; 
    }
...
bool dfs(string CURR_DFS_STATE, int pos) {
    // cout << CURR_DFS_STATE << ' ' << pos << ' ' << INPUT[pos] << endl;
    PATH.insert(CURR_DFS_STATE);
    ...
    go(nbrState, NFA[CURR_DFS_STATE][currentPosChar]) 
        isMem |= dfs(nbrState, pos + 1);
    return isMem;
}
...
signed main() {
    ...
    bool isMem = isMember();
    cout << "\"" << INPUT << "\""<< (isMem ? " is a member of NFA" : " is not a member of NFA") << endl;
